...
class Form(FieldValidationMixin):
	...
	def renderExtForm(self, page):
		
		form = FORM(method="Post", enctype="multipart/form-data", action=self._action, klass="x-form", id="form1", style="width: %s;" % self._width)
		divOuter = DIV()
		divOuter.append(DIV(DIV(DIV(klass="x-box-tc"),klass="x-box-tr"),klass="x-box-tl"))
		form.append(divOuter)
		divInner1 = DIV(klass="x-box-ml")
		divInner2 = DIV(klass="x-box-mr")
		divInnerContent = DIV(klass="x-box-mc")
		
		divInner1.append(divInner2)
		divInner2.append(divInnerContent)
		
		divInnerContent.append(H3(self._caption, style="margin-bottom:5px;"))
		divContainer = DIV(klass="x-form-bd", id="container")
		divInnerContent.append(divContainer)
		
		form.append(divInner1)
		
		form.append(DIV(DIV(DIV(klass="x-box-bc"),klass="x-box-br"),klass="x-box-bl"))
		
		currentFieldSet = divContainer
		fieldSetStack = Stack()
		fieldSetStack.push(currentFieldSet)
			...
			if control[1]<>"-":
				if control[1]<>"hidden":
					...
					if control[1]=="break":
						...
						formElement = DIV(klass="x-form-item")
						...
						if control[4]=="":
							#formItem.append(LABEL(control[0], label_for=control[2], style="width: %s" % labelWidth, onmouseover=mouseOver, onmouseout=mouseOut))
							formItem.append(LABEL(control[0], label_for=control[2]))
							#formItem.append(DIV(INPUT(type=control[1], name=control[2], value=control[3], style="width: %s" % width),klass="x-form-element"))
							formItem.append(DIV(INPUT(type=control[1], name=control[2], value=control[3]),klass="x-form-element"))
							if control[7]!="":
								#formItem.append(INPUT(type="input", value=control[7], style="padding-left: 0.5em; border: 1px; background-color: transparent;", readonly=1, border=1, size=control[8]))
								formItem.append(INPUT(type="input", value=control[7], readonly=1, border=1, size=control[8]))
							currentFieldSet.append(formItem)
						else:
							#formItem.append(LABEL(control[0], label_for=control[2], style="width: %s" % labelWidth, onmouseover=mouseOver, onmouseout=mouseOut))
							formItem.append(LABEL(control[0], label_for=control[2]))
							#formItem.append(DIV(INPUT(type=control[1], name=control[2], value=control[3], readonly=1, style="width: %s" % width),klass="x-form-element"))
							formItem.append(DIV(INPUT(type=control[1], name=control[2], value=control[3], readonly=1),klass="x-form-element"))
							if control[7]!="":
								#formItem.append(DIV(INPUT(type="input", value=control[7], style="border: 1px; background-color: transparent;", readonly=1, border=1, size=control[8]),klass="x-form-element"))
								formItem.append(DIV(INPUT(type="input", value=control[7], readonly=1, border=1, size=control[8]),klass="x-form-element"))
							currentFieldSet.append(formItem)
		...
		if self._haveButtonRow:
			...
			divContainer.append(buttonTable)
		...
		page.write(CENTER(form))

	def render(self, page):
		"""Render the form on a WebKit page."""
		
		self.renderExtForm(page)
		return
		...
		form = FORM(method="Post", enctype="multipart/form-data", action=self._action, klass="x-form")
...
class ExtForm(FieldValidationMixin,ExtEntity):
	"""Form class
	
	The form class implements a complete HTML input form that can be
	rendered on a WebKit page."""
	def __init__(self, page, name, action, caption="Noname", width=""):
		"""Class constructor
	...
	def addText(self, label, name, value = "", width="", labelWidth="", fieldType="string"):
		"""Add text control."""
		control = TextControl(name, label, value)
	...
	def getOnReadyInit(self):
		...
		for control in self._controls:
			if not isinstance(control, tuple):
				javaScriptString += control.renderJSToString() + "\n\n"
	...
	def _render(self):
		...
		for control in self._controls:
			...
			if not isinstance(control, tuple):
				currentFieldSet.append(control.renderToTag())
			else:
