...
class JobBasePage(ApplicationSecurePage):
        ...
        if self.session().hasValue("jobpage_status"):
            ...
            if self.request().hasField("editjob"):
                ...
                if sessionValid:
                    ...
                    form.addFormButton("Modify", "_action_modify")
                    form.addFormButton("Params...", "action_modifyParams")
                    form.addFormButton("Back", "_action_back")
                    form.setHaveSubmit(False)
    ...
    def actions(self):
        ...
        return ApplicationSecurePage.actions(self) + ["create", "modify", "modifyParams", "back", "uploadFile"]     
    ...
    def create(self):
        ...
        task.workDir = self.jobDir
            
        task.name = fieldValues["jobName"]

        if fieldValues.has_key("cpuTime"):
            task.cpuTime = fieldValues["cpuTime"]

        if fieldValues.has_key("email"):
            task.notify = fieldValues["email"]
        ...
        if self.onEditAfterCreate():
            self.session().setValue("jobpage_editing", task.name)
        else:
    ...
    def modify(self, confirmation = True):
        ...
        self.session().setValue("jobpage_task", task)
        self.session().setValue("jobpage_jobname", task.name)
        self.session().setValue("jobpage_jobdir", self.jobDir)
    ...
    def uploadFile(self):
        ...
        self.jobDir = userDir+"/job_%s" % (task.name)
        ...
        self.session().setValue("editjob", task.name)
        self.writeBody()        
    ...
    def onCreateNewJobForm(self, task):
        ...
        form = Web.Ui.Form("newJobForm", self.onGetPageAddress(), "Create an %s job" % (task.description), width="26em")
        
        #attribs = task.getAttributes()
        #xrslAttribs = task.getXRSLAttributes()
        
        #form.beginFieldSet("Information")
        form.addText("Job name", "jobName", task.name, width="20", labelWidth="12em")
        form.addBreak()
        #form.endFieldSet()
