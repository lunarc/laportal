...
import Lap
import Web
import Plugins.Matlab.CustomJob
import Lap.Utils
...
class CustomJobPage(JobPage):
	...
	def onCreateNewTask(self):
		...
		task = Plugins.Matlab.CustomJob.CustomTask()
	...
	def onCreateEditJobForm(self, task):
		...
		form = Web.Ui.Form("editJobForm", self.pageLoc()+"/Plugins/%s/CustomJobPage" % (pluginName), "Edit MATLAB job", width="30em")
		...
		form.setControlHelp("mainFile", "Select the MATLAB main file to be used using the Browse button.")
