...
class User(object):
	...
	"""
	def __init__(self, userId="", DNString=""):
		"""Class constructor.
		...
		if userId!="":
			self._loginType = "userid"
			self._userId = userId
		
		self._sessionDir = LapSite.Dirs["SessionDir"]

		if self._loginType == "DN":
	...
	def setupUser(self):
		...
		self._userDirname = self._userId
		self._userDir = os.path.join(self._sessionDir,"%s" % (self._userDirname))
	...
	def createDir(self):
		...
		if self._loginType == "DN":
			...
			if self._userId!=None:
				if not os.path.exists(self._userDir):
					self.checkAndMkdir(self._userDir)
					lapInfo("Creating user directory: "+self._userDir)
					self.savePrefs()
	...
	def exists(self):
		...
		if self._loginType == "DN":
			...
			return os.path.isdir(os.path.join(self._sessionDir, self._userDir))
			
	def setProxyPermissions(self):
		os.chmod(self.proxyFilename, 0600)
	...
	def getProxy(self):
		...
		return self.getProxyFilename()
		
	def getProxyFilename(self):
		"""Return user proxy filename."""
		return self._userDir+"/proxy.pem"
	...
	proxyFilename = property(getProxyFilename)
